...
Com relação ao comportamento da função de risco, quando $\lambda$ é menor ou igual a 1, esta é monótona decrescente, enquanto para valores maiores que 1 a função cresce até um valor máximo e após isso tem um comportamento decrescente.
...
h(t|x) = - \dfrac{d}{dt}\log{\frac{1}{1 + \left(\frac{t}{\boldsymbol{x}^T\boldsymbol{\beta}}\right)^{\lambda}}}, \hspace{0.75cm}  t > 0, \lambda > 0, -\infty < \boldsymbol{\beta} < \infty,
\end{equation}
...
\hspace{24pt}Para modelagem deste banco de dados, será realizada uma adaptação mais fraca do método *forward* de seleção de variáveis. Assim, será realizada a estimação do modelo apenas com o intercepto e após isso, serão estimados modelos adicionando variáveis uma a uma e escolhendo a variável mais significativa. A seleção pára quando não tiver mais nenhuma variável significativa.
